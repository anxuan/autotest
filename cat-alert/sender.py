...
class sender_mail(sender):
    ...
    def send(self):
        ...
        try:
            ...
            # 默认25号端口，改用这个端口号以绕过阿里云对25号端口的封禁
            # https://yq.aliyun.com/articles/661982?spm=a2c4e.11155472.0.0.45b27ea8oAO5jV
            smtp = smtplib.SMTP(port=465)
            ...
            logging.error('mail send exception -> %s', str(e))
...
class sender_weixin(sender):
    ...
    def get_token(self):
        if len(weixin_token_dict) == 0:
            response = urlopen(self.url_gettoken)
    ...
    def send(self):
        ...
        try:
            ...
            req = Request(url=send_url, data=data_dict, headers=header_dict)
            res = urlopen(req)
            send_res = res.read()
            ...
            logging.error('weixin send exception -> %s', str(e))
